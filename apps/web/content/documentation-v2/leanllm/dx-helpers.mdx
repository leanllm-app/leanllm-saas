---
title: "DX Helpers"
description: "Module-level init, last-event ring buffer, pretty printers, auto-chain, and trace helpers."
...
- **`last_event` is process-local.** It's a `collections.deque` of `LLMEvent`s — not shared across processes, not durable.
- **Auto-chain advances only on emitted events.** Sampled-out events don't advance the chain — see [runtime toggles](/docs-v2/leanllm/runtime-toggles) for the consequence.
