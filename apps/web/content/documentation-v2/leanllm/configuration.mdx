---
title: "Configuration"
description: "Every LeanLLMConfig field and its env var — the canonical reference."
...
In-memory retry only. No disk fallback by design — see the rationale in the [request interception page](/docs-v2/leanllm/interception).
...
| `redaction_mode` | `LEANLLM_REDACTION_MODE` | `metadata` | `metadata` / `hashed` / `full`. See [redaction](/docs-v2/leanllm/redaction). |

## Normalization

| Field | Env var | Default | What it does |
|---|---|---|---|
| `auto_normalize` | `LEANLLM_AUTO_NORMALIZE` | `false` | Populate `normalized_input` / `normalized_output` on every event. See [normalization](/docs-v2/leanllm/normalization). |
...
See [runtime toggles](/docs-v2/leanllm/runtime-toggles) for per-call overrides (`log=False`, `sample=...`, `redaction_mode=...`).
