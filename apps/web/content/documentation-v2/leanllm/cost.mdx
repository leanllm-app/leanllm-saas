...
description: "USD cost computed from a built-in pricing table; token estimates when the provider doesn't return usage."
...
- [Storage query API](/docs-v2/leanllm/storage-query) — query by model, sum cost.
- [Lineage](/docs-v2/leanllm/lineage) — `subtree_cost` rolls up across a flow.
